...
class HerbariumImportTemporaryPreviewTest extends TestCase
{
    ...
    public function test_batch_importer_does_not_generate_or_render_temporary_previews(): void
    {
        ...
        $this->assertArrayNotHasKey('previewUrls', $viewData);
        $this->assertArrayNotHasKey('preview_url', $rows[$rowKey]);
        $this->assertArrayNotHasKey('preview_expires_at', $rows[$rowKey]);
        $component
            ->assertSee('13536.jpg')
            ->assertDontSee('/herbarium/images/import-preview/', escape: false)
            ->assertDontSee('Temporary preview');

        $template = file_get_contents(resource_path('views/livewire/herbarium/import-images.blade.php'));
        $this->assertIsString($template);
        $this->assertStringNotContainsString('<img', $template);
        $this->assertStringNotContainsString('previewUrls', $template);
    }

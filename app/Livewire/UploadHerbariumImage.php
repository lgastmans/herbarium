...
class UploadHerbariumImage extends Component
{
    ...
    public function save()
    {
        ...
        $filename = uniqid() . '_' . $this->photo->getClientOriginalName();

        $this->photo->storeAs('herbarium', $filename, 'public');

        $model = HerbariumImages::create([
            'herbarium_id' => $this->herbarium_id,
    ...
    public function save()
    {
        ...
        if ($this->photo) {
            ...
            logger()->info('Upload temp path', [
                'exists' => $this->photo->exists(),

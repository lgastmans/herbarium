...
//        $this->persist(['columns', 'filters'], prefix: auth()->id ?? '');       
    ...
    public function datasource(): Builder
    {
        ...
        $query = Herbarium::with(['family', 'genus', 'place', 'collector1', 'collector2', 'collector3']);
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Family', 'family_name') // relationship.column
                ->sortable()
                ->searchable()
                ->visibleInExport(visible: true),

            Column::make('Genus', 'genus_name') // relationship.column
                ->sortable()
                ->searchable()
                ->visibleInExport(visible: true),

            Column::make('Place', 'place_name') // relationship.column
                ->sortable()
                ->searchable()
                ->visibleInExport(visible: true),
            ...
            Column::make('Collected By', 'collected_by')
                ->html()
                ->filterSelect(
                    Collector::orderBy('name')->pluck('name', 'id')->toArray(),
                    'collector_filter'
                ),
            */

/*
Collector::all()->mapWithKeys(function ($c) {
    return [$c->id => $c->name . ' ' . ($c->surname ?? '')];
})->toArray();
*/
    ...
    public function filters(): array
    {
        ...
        return [
            ...
            Filter::select('family_id')
                ->dataSource(Family::query()->orderBy('family', 'asc')->get())
                ->optionLabel('family')
                ->optionValue('id')
                ->label('Family'),

            Filter::inputText('place.name')
                ->label('Place'),
